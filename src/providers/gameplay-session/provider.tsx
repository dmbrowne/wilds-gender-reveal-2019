...
const GameplaySessionProvider: React.FC<IWithSessionContext> = ({ children }) => {
  ...
  const [questionsByGameId, setQuestionsByGameId] = useState<{ [key: string]: any }>({});
  ...
  const updateQuestionStatus = (questionId: string, gameId: string, data: { [key: string]: any }) => new Promise((resolve, reject) => {
  ...
  const markQuestionAsComplete = (questionId: string, gameId: string) => {
    updateQuestionStatus(questionId, gameId, { completedAt: new Date() });
    if (gameplaySession) {
  ...
  const unlockQuestion = (questionId: string, gameId: string) => {
    ...
    return updateQuestionStatus(questionId, gameId, { unlockedAt: new Date() });
  }

  const markGameAsComplete = (gameId: string) => {
    updateGameSession(gameId, { completedAt: new Date() });
    if (gameplaySession) {
  ...
  useEffect(() => {
    ...
    Promise.all(Object.keys(gameplaySession.games).map(gameId => fetchGameData(gameId)))
      .then((documentSnapshots) => {
        const fetchedGames = documentSnapshots.reduce((accum, snapshot) => ({
          ...accum,
