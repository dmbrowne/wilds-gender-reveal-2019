...
import GenderPredictionContainer from "../../containers/gender-prediction/gender-prediction";

interface IProps extends RouteComponentProps<{ gameId: string }>, IWithGameplaySessionContext {}

const getCustomGameScreen = (game: IGame) => {
  switch (game.slug) {
    case "gender-prediction":
      return () => <GenderPredictionContainer />;
  }
};
const getGameScreen = (gametype: string, game: IGame, questions: any, questionsStatus: any) => {
  switch (gametype) {
    case "custom":
      return getCustomGameScreen(game);
    case "qanda":
      return () => <ScavengerHunt game={game} questions={questions} questionsStatus={questionsStatus} />;
    case "snowman":
