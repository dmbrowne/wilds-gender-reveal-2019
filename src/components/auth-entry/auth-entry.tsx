    ...
    <div className={styles.container}>
      {formError && (
        <Paper
          ...
          elevation={0}
          style={{ background: theme && theme.palette.error.dark }}
        >
          <ErrorIcon /> {formError}
        </Paper>
      )}
      {entryMode === "signup" && (
        <>
          ...
          <Button size="small" color="secondary" onClick={() => changeEntryMode("signin")}>
      ...
      {entryMode === "signin" && (
        <>
          ...
          <Button size="small" color="secondary" onClick={() => changeEntryMode("signup")}>
            Haven't yet registered? sign up
          </Button>
        </>
      )}
    </div>
