...
class ScavengerHuntContainer extends React.Component<IProps>{
  ...
  addCode = (code: string) => new Promise((resolve, reject) => {
    ...
    return gameplaySessionContext.unlockQuestion(question.id, match.params.gameId)
      .then(() => resolve())
      .catch(() => reject());
  })

  render() {
    const { game, questions, questionsStatus, gameplaySessionContext, match } = this.props;
    ...
    const { markQuestionAsComplete, unlockNextQuestion, gameplaySession } = gameplaySessionContext;
    const pointsAvailable = gameplaySession && gameplaySession.games[gameId].pointsAvailable || 0;
    const complete = gameplaySession && !!gameplaySession.games[gameId].completedAt || false;
...
const ScavengerHuntGame = compose(withRouter, withGameplaySession)(ScavengerHuntContainer);
